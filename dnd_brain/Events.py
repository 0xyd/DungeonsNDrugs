...
class PoliceEvent():
	...
	def show_event(self):
		...
		if self.event == '臨檢事件':
			return ('開車遭臨檢:', '開車行經中山路，遇到前方有警員設路障盤查，前方排隊車龍還長，心中盤算究竟該如何是好...》', 'http://i.imgur.com/NKAEkcT.png')
		elif self.event == '檢調搜索':
			return ("檢調搜索", "你因為事業越做越大，開始受到警方關注，警方除佈下人力跟監，亦順利聲請通訊監察，而你卻毫無察覺...某日，尖銳的電鈴聲劃破早晨的寧靜，警察：「我們是偵查隊刑警，依檢字106年0000689號搜索票執行搜索，請開門配合搜索，謝謝。」驚惶失措的你，只有幾秒反應時間...", "http://i.imgur.com/P0EVkYl.png")
		else:
			return "其他事件"
...
class GangsterEvent():
	...
	def show_event(self):
		...
		if self.event == '黑幫霸凌':
			return ('黑幫霸凌', "身為校園邊緣人的你，加入幫派後依然沒有例外，22k幫向以財力作為地位高低衡量，窮困潦倒的你，連菜鳥都把你當小弟使喚，面對如此困境，你決定...", "http://i.imgur.com/rycGQRj.png")
		else:
			return ("其他事件", "其他事件描述")
	...
	def _be_bully(self, dice_value):
		...
		if dice_value == 1:
			...
			return (0, money_loss, -15, 0, '任命不願抵抗，生無大志只求小確幸，過一日算一日', False)
		elif dice_value < 10:
			return (0, 0, 0, 0, '忍辱負重，持續耕耘，等待翻身的一天', False)
		else:
			return (20, 0, 0, 0, '奮力抵抗體制，雖歷經一番掙扎、多了幾道傷疤，但終獲尊重', False)
	...
	def _be_kinapped(self, dice_value):
		if dice_value > 4:
			return (0, 0, 0, 0,'成功掙脫且逃逸', False)
		else:
			return (0, 0, 0, 0,'支付贖金。', False)
